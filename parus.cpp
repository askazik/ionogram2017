...
namespace parus {
    ...
    // ==================================================================================================
		...
		header.freq_min = ion->getFreq_min(); // начальная частота, кГц (первого модуля)
		header.freq_max = ion->getFreq_max(); // конечная частота, кГц (последнего модуля)   
		header.count_freq = ion->getFreq_count(); // число частот во всех модулях
		header.count_modules = 1; // количество модулей зондирования
